...
packer build -only=virtualbox-iso.default \
  -var-file=vars-alpine-standard-3.22-x86_64.pkrvars.hcl \
  alpine-3.22-minimal.pkr.hcl
```
...
* `alpine-3.22-minimal.pkr.hcl` - Alpine Linux 3.22.1
* `alpine-3.22-docker.pkr.hcl` - Alpine Linux 3.22.1 with [Docker] and
  [Docker Compose]
* `alpine-3.22-dwm.pkr.hcl` - Alpine Linux 3.22.1 with [X.org][],
  [dwm][], [dmenu][], [st][], [SLiM][], and [xrdp][].
* `alpine-3.22-xfce.pkr.hcl` - Alpine Linux 3.22.1 with [Xfce][] and
  [xrdp][].
...
Instead of `vars-alpine-standard-3.22-*.pkrvars.hcl` files you can use
`vars-alpine-virt-3.22-*.pkrvars.hcl`.  These files instruct to use ISO
