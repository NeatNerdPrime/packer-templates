...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "CPU=${var.cpu}",
      "OPEN_VM_TOOLS=open-vm-tools=12.5.0-r0",
      "OS_VER=v${var.os_ver}"
    ]
    only = [
      "vmware-iso.*"
  ...
  provisioner "shell" {
    environment_vars = [
      "NFS_UTILS=nfs-utils=2.6.4-r4",
      "UTIL_LINUX=util-linux=2.41-r9"
    ]
    only = [
      "qemu.default"
    ]
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "OS_VER=v${var.os_ver}",
      "PYTHON_PIP=py3-pip=25.1.1-r0",
      "PY_BCRYPT=py3-bcrypt=4.3.0-r0",
      "PY_CRYPTOGRAPHY=py3-cryptography=44.0.3-r0",
      "PY_PYNACL=py3-pynacl=1.5.0-r6",
      "SUDO_CMD=doas",
