...
locals {
  boot_command = [
    "root<enter><wait>",
    "/usr/sbin/setup-interfaces -i << EOF<enter>",
    "auto lo<enter>",
    ...
    "DISK=%s ERASE_DISKS=/dev/%s /usr/sbin/setup-alpine -f /tmp/answers.txt<enter><wait><wait10>",
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "OS_VER=v${var.os_ver}",
      "VIRTUALBOX_GUEST_ADDITIONS=virtualbox-guest-additions=7.0.22-r0"
    ]
    only = [
      "virtualbox-iso.*"
    ]
  ...
  provisioner "shell" {
    environment_vars = [
      "CPU=${var.cpu}",
      "OPEN_VM_TOOLS=open-vm-tools=13.0.0-r1",
      "OS_VER=v${var.os_ver}"
    ]
    only = [
      "vmware-iso.*"
  ...
  provisioner "shell" {
    environment_vars = [
      "NFS_UTILS=nfs-utils=2.6.4-r6",
      "UTIL_LINUX=util-linux=2.41.2-r0"
    ]
    only = [
      "qemu.default"
    ]
  ...
  provisioner "shell" {
    environment_vars = [
      "DOCKER=docker=${var.docker_version}",
      "DOCKER_COMPOSE=docker-cli-compose=${var.docker_compose_version}",
      "OS_VER=v${var.os_ver}",
      "PYTHON_PIP=py3-pip=25.1.1-r1",
      "PY_BCRYPT=py3-bcrypt=5.0.0-r0",
      "PY_CRYPTOGRAPHY=py3-cryptography=46.0.5-r0",
      "PY_PYNACL=py3-pynacl=1.6.2-r0",
      "SUDO_CMD=doas",
