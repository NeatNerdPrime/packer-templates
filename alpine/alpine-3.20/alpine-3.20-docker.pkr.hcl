...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      "OS_VER=v${var.os_ver}",
      "VIRTUALBOX_GUEST_ADDITIONS=virtualbox-guest-additions=7.0.18-r0"
    ]
    only = [
      "virtualbox-iso.*"
    ]
  ...
  provisioner "shell" {
    environment_vars = [
      "CPU=${var.cpu}",
      "OPEN_VM_TOOLS=open-vm-tools=12.4.0-r0",
      "OS_VER=v${var.os_ver}"
    ]
    only = [
      "vmware-iso.*"
  ...
  provisioner "shell" {
    environment_vars = [
      "NFS_UTILS=nfs-utils=2.6.4-r1",
      "UTIL_LINUX=util-linux=2.40.1-r1"
    ]
    only = [
      "qemu.default"
    ]
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "PY_CRYPTOGRAPHY=py3-cryptography=42.0.7-r0",
