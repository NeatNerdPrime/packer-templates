...
variable "iso_checksum" {
  type        = string
  default     = "file:https://mirror.leaseweb.com/devuan/devuan_excalibur/installer-iso/SHA256SUMS.txt"
  description = "SHA256 checksum of the install media."
}

variable "iso_name" {
  type        = string
  default     = "devuan_excalibur_6.1.0_amd64_server.iso"
  description = "File name of the install media."
}

variable "iso_path" {
  type        = string
  default     = "devuan_excalibur/installer-iso"
...
locals {
  ...
  vm_name = coalesce(var.vm_name, "Devuan-6-${var.cpu}-dwm")
  vmware_vmx_data = {
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "DEVUAN_VERSION=excalibur",
      "DWM=dwm=6.5-1",
      "INSTALL_FROM_DVD=${var.install_from_dvd}",
      "STTERM=stterm=0.9.2-1",
      "SUCKLESS_TOOLS=suckless-tools=48-1+b1",
  ...
  provisioner "shell" {
    ...
    script       = "../provisioners/virtualbox_72.sh"
    pause_before = "10s"
  }

  provisioner "shell" {
    environment_vars = [
      "OPEN_VM_TOOLS_DESKTOP=open-vm-tools-desktop=2:12.5.0-2+deb13u1",
      "VMWARE_WITH_XORG=1",
